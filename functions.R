...
#/*=================================================*/
make_trial_grids <- function(field, ab_line, starting_point, plot_width, subplot_length) {
...
month_prec_graph <- function(weather, comp_year){
  ...
  monthprec_avg_not_2018 <- avgprec_by_month(subset(by_month_year, year != comp_year))
  monthprec_2018 <- subset(by_month_year, year == comp_year)
  ...
  monthly_prec <- ggplot(prec_merged) +
    ...
    ggtitle(paste0(comp_year, " Monthly Precipitation Compared to Average")) +
    labs(y= "Precipitation (in)", x = "Month") +
    guides(fill=guide_legend(title=paste0(comp_year, " Minus Average")))
  return(monthly_prec)
}

trial_design <- function(boundary, abline, width, length, rates, file_name){
  square_field <- boundary %>%
    st_bbox() %>%
    st_as_sfc() %>%
    st_as_sf()
  ...
  subplot_length <- conv_unit(length/20, "ft", "m")
  polygon_width <- width 
  
  all_grids <- make_trial_grids(square_field, abline, starting_point, polygon_width, subplot_length)
  design <- assign_rates(all_grids, rates, merge = TRUE)
  
  st_write(design, file_name, delete_layer = TRUE)
  return(design)
}
